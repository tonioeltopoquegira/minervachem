...
def load_data_per_solvent(pkl_fn='/Users/ypimonova/Documents/mdspace/rscheck_acetone_sol.pkl', solvent='acetone'):
...
def random_seed_check_r2_errorbars(pkl_fn='/Users/ypimonova/Documents/mdspace/rscheck_acetone_sol.pkl',
...
def alpha_perp_par_plot(pkl_fn='/Users/ypimonova/Documents/mdspace/mdspace_hp_gridsearch.pkl',
                           figsize=(17.5, 7.5), dpi=500, suffix=''):
    ...
    for i in range(len(properties)):
        ...
        plt.tight_layout()
        plt.savefig(f'/Users/ypimonova/Documents/mdspace/alphasratios_{properties[i]}{suffix}.png')
        plt.clf()
    return


# def get_result_for_task_witherrorbar(solv_name, filelocation, ns=50, maxssize=5, err='rmse'):
#     solvdata = load_data_per_solvent(pkl_fn=filelocation, solvent=solv_name)
#     d = solvdata[(solvdata.max_subgraph_size == maxssize) & (solvdata.n_shots == ns)].reset_index(drop=True)
...
def plot_relative_improvement(filelocation, task_name, metrics=('rmse', 'r_2', 'mae'),
    ...
    for row, metric in enumerate(metrics):
        ...
        # ax.set_title(f'Relative Improvement in {metric.upper()}', fontsize=18, weight='bold')
        # ax.set_xlabel('N shots', fontsize=14, weight='bold', labelpad=5)
        if metric == 'r_2':
...
# def get_relative_improvement_witherrorbar(task_name, filelocation, ns=20, maxssize=5, err='rmse', verbose=False):
#     """
#     This function loads result data from the specified file and filters the dataset based on
#     the provided number of shots (ns) and/or maximum subgraph size (maxssize). It then extracts the
#     performance metrics for both meta and non-meta approaches, finds the relative improvement according to the following formula:
#     (NM/M - 1)*100% and calculates the mean and standard error over the random seeds, and returns these values in a dictionary.
#     :param task_name: str
#         Identifier for the task.
#     :param filelocation: str
#         Path to the file containing the result data.
...
#         for el2 in non_meta:
#             if err == 'r_2':
#                 improvement.append(relative_improvement(el1, el2), reverse=False)
#     improvement = np.array(improvement)
#     improvement_mean = np.mean(improvement, axis=0)
#     improvement_std = np.std(improvement, axis=0) / np.sqrt(len(improvement))
#     return improvement_mean, improvement_std
...
def construct_filename(taskname, value_type='logmolar', maxsize=5, nlayers=None):
    if nlayers==0:
        fn = f'/uufs/chpc.utah.edu/common/home/u1262027/lanl_research/metalearn_md/llns100_13solvents_maxsize{maxsize}/' \
...
def meta_nonmeta_parity(tasks_list=(), nshots=50, maxsize=5, nlayers=0, err='rmse', figsize=(6, 6)):
    ...
    figure_name = f'/uufs/chpc.utah.edu/common/home/u1262027/lanl_research/metalearn_md/llns100_13solvents_maxsize{maxsize}/' \
...
def meta_nonmeta_parity_allgenoptions(tasks_list=(), nshots=50, maxsize=5, err='rmse', figsize=(6, 6)):
    ...
    for i, name in enumerate(names):
        ...
        # ax.annotate(name, (x2[i], y2[i]), textcoords="offset points", xytext=(5, 5), ha='center', fontsize=7)
        # ax.annotate(name, (x5[i], y5[i]), textcoords="offset points", xytext=(5, 5), ha='center', fontsize=7)
    ax.plot([0, 1], [0, 1], 'b--', transform=ax.transAxes)
    if err=='rmse':
        ax.set_xlabel('RMSE, non-meta')
        ax.set_ylabel('RMSE, meta')
...
def meta_nonmeta_parity_allgens(tasks_list=(), nshots=50, maxsize=5, eps=0.2, err='rmse', figsize=(6, 6)):
    ...
    plt.legend(handles, labels, loc='best')
    figure_name = f"/uufs/chpc.utah.edu/common/home/u1262027/lanl_research/metalearn_md/llns100_13solvents_maxsize{maxsize}/" \
                  f"{err}_meta_nonmeta_allgens_{nshots}shots_eps{str(eps).replace('.', '')}_23Jan24.png"
    plt.tight_layout()
    plt.savefig(figure_name)
...
def values_distributions(filename, make_log=False, fig_suffix='', figsize=(6, 6), xleft = 0.0, xright=10.0):
    ...
    # Show plot
    plt.tight_layout()
    plt.savefig(f'/uufs/chpc.utah.edu/common/home/u1262027/lanl_research/metalearn_md/'
                f'value_freqs_llns100_for_{fig_suffix}.png')
