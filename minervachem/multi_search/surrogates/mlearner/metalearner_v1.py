...
# TODO add docstrings to all functions of the metalearner class


class MetaLearner:
    def __init__(self, max_subgraph_size=5, random_state=16, epsilon_par=1.0,
                 epsilon_perp=1.0, hierarchical=False, make_log=False, a_range='a4'): # database_path, working_directory=None,  n_shots=20, 
    ...
    def calculate_residuals(self, model_vectors, t_bar, new_task_nshots):
        # assert self.n_ots == len(
        #     model_vectors), "Number of independent models vectors does not match the number of old tasks" # TODO fails multilayer, fix that
        # Getting residuals of predictions of old tasks (based on independent models) for molecules from new task nshots

        X = new_task_nshots[0] 
        Y = new_task_nshots[1]

        task_residuals = []
        # for task_id in range(self.n_ots):
        #t = time.time()
        for task_id in range(len(model_vectors)):
            ...
            e_temp = X.dot(theta_temp).flatten()
            task_residuals.append(e_temp)
        
        #print(f"For loop time {time.time() - t}")
        e_residuals = np.vstack(task_residuals).T
        # The residuals of the new task are using real values instead of predictions
        #t = time.time()
        residuals_for_new_task = Y - X.dot(t_bar).flatten()
        #print(f"Other comp time {time.time()-t}s")
        return e_residuals, residuals_for_new_task

    '''def solve_parallel(self, model_vectors, t_bar, new_task_nshots, sample_weight=None, print_alpha=True, featurizer=None):
        #t = time.time()
        eres, ress_nt = self.calculate_residuals(model_vectors, t_bar, new_task_nshots) # this is the main driver of computation
        #print(f"Calculated residuals in {time.time()-t}s", flush=True)

        p_par, _ = cross_validate_parameters(eres, ress_nt, alpha_range='a_parallel', sample_weight=sample_weight, featurizer=featurizer)
        
        model_par = Ridge(**p_par)
        if self.hierarchical:
            hmodel_par = HierarchicalResidualModel(regressor=model_par, verbose=1)
            hmodel_par.fit(eres, ress_nt, levels = featurizer.bit_sizes_)
            c_is = np.copy(hmodel_par.coef_)
            if print_alpha:
                print(f'Alpha for parallel solution: {hmodel_par.alpha}')
        else:
            
            model_par.fit(eres, ress_nt, sample_weight=sample_weight)
    ...
    # ---------- replace existing solve_parallel ----------
    def solve_parallel(self, model_vectors, t_bar, new_task_nshots, sample_weight=None, print_alpha=True, featurizer=None):
        """
        Solve the parallel contribution. Returns (t_is, parallel_alpha).
        Stores only minimal diagnostics on the instance:
        - self.last_parallel_c_is  (the c coefficients used to form t_par)
        - self.last_parallel_norm  (L2 norm of t_par)
        """
        ...
        # compute parallel contribution in feature space
        t_is = np.dot(c_is, np.asarray(model_vectors) - t_bar)

        # store diagnostics: c vector and norm of parallel component
        try:
            self.last_parallel_c_is = np.asarray(c_is).copy()
        except Exception:
            self.last_parallel_c_is = None
        try:
            self.last_parallel_norm = float(np.linalg.norm(t_is))
        except Exception:
            self.last_parallel_norm = None

        return t_is, getattr(model_par, 'alpha', None)


    def solve_perpendicular(self, new_task_nshots, t_bar, t_par, print_alpha=True, sample_weight=None, featurizer=None):
        """
        Solve perpendicular contribution. Returns (t_perp, theta_task, alpha_perp)
        Stores minimal diagnostics:
        - self.last_perp_vector
        - self.last_perp_norm
        """
        perp_par, _ = cross_validate_parameters(new_task_nshots[0], new_task_nshots[1], alpha_range='a4', sample_weight=sample_weight, featurizer=featurizer)
        model_perp = Ridge(**perp_par)
        tt = t_bar + t_par
        if self.hierarchical:
            hmodel_perp = HierarchicalResidualModel(regressor=model_perp, verbose=1)
            fit_with_prior(ridgemodel=hmodel_perp, hierarchical=self.hierarchical, x_values=np.asarray(new_task_nshots[0].todense()), y_values=new_task_nshots[1],
                        prior=tt, a_range=self.alpha_range, featurizer=featurizer)
            theta_task = np.copy(hmodel_perp.coef_)
        else:
            theta_task, alpha_perp = fit_with_prior(model_perp, sample_weight, self.hierarchical, new_task_nshots[0],
                                                    new_task_nshots[1],
        ...
        try:
            self.last_perp_vector = np.asarray(t_perp).copy()
        except Exception:
            self.last_perp_vector = None
        try:
            self.last_perp_norm = float(np.linalg.norm(t_perp))
        except Exception:
            self.last_perp_norm = None
    ...
    def fit_base(self, task, featurizer=None, sample_weight=None):

        #t = time.time()
        coeff_base, params_base = run_default_ridge_regressions(task, sample_weight=sample_weight, a_range=self.alpha_range, hierarchical=self.hierarchical, featurizer=featurizer)
        #print(f'solved base in {time.time()-t}s', flush=True)

        error_test = evaluate_regression_model(params_base[0], coeff_base[0], task[0][0], task[0][1], print_on=False) # Modified
        error_train = evaluate_regression_model(params_base[0], coeff_base[0], task[0][0], task[0][1], print_on=False)
        

        self.coeff = coeff_base[0]

        self.params = params_base

        return error_train['rmse'], error_test['rmse'], self.coeff, self.params


    def fit_meta(self, task, other_coeff, other_params, sample_weight = None, featurizer=None):

        # new step - orthonormalization of the set of parvecs for old tasks
       
        parvecs_ortho = gram_schmidt(np.array(other_coeff))
        ...
        #t = time.time()
        theta_parallel, parallel_alpha = self.solve_parallel(other_coeff, theta_bar, task[0], print_alpha=False, sample_weight=sample_weight, featurizer=featurizer)
        #print(parallel_alpha)
        #print(f'solved parallel in {time.time()-t}s', flush=True)

        #print('parallel done', flush=True)
        # Calculating theta_perpendicular
        #t = time.time()
        theta_perpendicular, theta_task, perpendicular_alpha = self.solve_perpendicular(task[0], theta_bar, theta_parallel, print_alpha=False, sample_weight=sample_weight, featurizer=featurizer)
        #print(perpendicular_alpha)
        #print(f'solved perp in {time.time()-t}s', flush=True)

        #print('perpendicular done', flush=True)

        # Update the internal coeff and params
        self.coeff = theta_task
        self.params = other_params

        error_test = evaluate_regression_model(other_params[0], theta_task, task[0][0], task[0][1],  print_on=False) #modified
        error_train = evaluate_regression_model(other_params[0], theta_task, task[0][0], task[0][1],  print_on=False)
        return error_train['rmse'], error_test['rmse']

    # 03-1690-118-ConfRoom-cnls_maya_lounge 03-0123-251-ConfRoom-T_DO_Harlow 03-0508-211A-ConfRoom-T_5_SM508_Rm_211A_Confere



    def predict(self,X):

        model_temp = Ridge(**self.params[0])
        model_temp.coef_ = self.coeff
    ...
    def multi_layer_model(self, n_layers=2, dumpfreq=1, more_notes='',  # Do not change dumpfreq!!!! IF YOU skip layers, meta updates will be skipped too -> something to fix later
        ...
        for layer in layers:
            ...
            if layer in layers_to_save: # TODO change to all layers to save/calc
                ...
                #     task_vectors=t_spec_vectors,
                #     resnotes=f'layer{layer}_targettask_{more_notes}', save_res_to=None)
            ...
            for size in mss_range:
                ...
                for n in nshots_range:
                    ...
                    for sd in rss_range:
                        ...
                        try:
                            # _, rd = self.single_layer_model(resnotes=f'{rnotes}_rs{int(sd)}', save_res_to=None) # 12/12 working on epsilon
